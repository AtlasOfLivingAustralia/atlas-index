...
test.describe('App.tsx', () => {
    test('renders the Sensitive Data Service page at / once CSS is loaded', async ({ page }) => {
    ...
    test('login button is present and handleLogin is callable', async ({ page }) => {
        ...
        const loginBtn = page
            .locator('a[href*="userdetails"], button, .loginBtn')
    ...
    test('footer renders alongside handleLogoutWrapper', async ({ page }) => {
        await setupMocks(page, setupDefaultMock);
        await page.goto(BASE_URL + '/');
        await page.waitForLoadState('networkidle');
