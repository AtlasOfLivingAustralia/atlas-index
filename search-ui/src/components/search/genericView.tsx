...
function GenericView({
    ...
    const [sortData, _] = useState<any[]>(() => {
        ...
        if (!props.sortByDate) {
            ...
            list = list.filter(
                (item) => item.value !== '&sort=created&dir=desc'
            );
        }
        return list;
    });
    ...
    function getResults(
        ...
        const thisSortValue =
            overrides?.sortParam !== undefined
                ? overrides.sortParam
                : sortValue;
        const thisPage = overrides?.page !== undefined ? overrides.page : page;
        const thisFacetFqs =
            overrides?.facetFqs !== undefined ? overrides.facetFqs : facetFqs;
        console.log(
            'page',
            overrides,
            thisPage,
        ...
        const url =
            ...
            '/v2/search?q=' +
            encodeURIComponent(queryString as string) +
            '&facets=' +
            Object.keys(props.facetDefinitions).join(',') +
            (props.customFacets ? ',' + props.customFacets.join(',') : '') +
            (thisFacetFqs.length > 0
                ? '&fq=' +
                  thisFacetFqs.map((fq) => encodeURIComponent(fq)).join('&fq=')
                : '') + // the 'drill down' does not update the facets section
            '&pageSize=' +
            pageSize +
            '&fq=' +
            encodeURIComponent(props.fq) +
            '&page=' +
            thisPage +
            thisSortValue;
            ...
            .then((data) => {
                ...
                if (data.searchResults) {
                    ...
                    if (
                        ...
                        Object.keys(props.facetDefinitions).forEach(
                            (facetName: string) => {
                                var facet = data.facetResults.find(
                                    (f: any) => f.fieldName == facetName
                                );
                                if (facet) {
                                    const parseFacetFn =
                                        props.facetDefinitions[facetName]
                                            .parseFacetFn;
                                    ...
                                    if (parseFacetFn) {
                                        ...
                                        if (items.length > 0) {
                                            ...
                                            facetList.push({
                                                name: props.facetDefinitions[
                                                    facet.fieldName
                                                ].label,
                                                items: items,
                                                order: props.facetDefinitions[
                                                    facet.fieldName
                                                ].order,
                                            });
                                        }
                        ...
                        if (!getFacets) {
                            ...
                            facetCopy.forEach((existingFacet: any) => {
                                var newFacet = facetList.find(
                                    (f: any) => f.name == existingFacet.name
                                );
                                if (newFacet) {
                                    existingFacet.items.forEach(
                                        (existingItem: any) => {
                                            var newItem = newFacet.items.find(
                                                (i: any) =>
                                                    i.fq == existingItem.fq
                                            );
                                            if (newItem) {
                                                existingItem.count =
                                                    newItem.count;
                        ...
                        if (props.addCustomFacetsFn) {
                            props.addCustomFacetsFn({
                                ...
                                thisFacetFqs,
                                parentData: data,
                                setCustomFacetData: (newData: any[]) =>
                                    applyCustomFacetData(
                                        newData,
                                        thisFacetFqs,
                                        getFacets
                                    ),
            ...
            customFacetData.forEach((existingFacet: any) => {
                ...
                if (newFacet) {
                    ...
                    existingFacet.items.forEach((existingItem: any) => {
                        ...
                        existingItem.selected = thisFacetFqs.includes(
                            existingItem.fq
                        );
                    });
                } else {
    ...
    const refineResults = (
        <>
            {(loading || maxResults > 0) &&
                queryString &&
                facets.length + customFacetData.length > 0 && (
        ...
        <>
            ...
            <div className="row">
                {isMobile ? (
                    ...
                    <div className="col-3">{refineResults}</div>
                )}
                <div className={isMobile ? 'col-12' : 'col-9'}>
                    ...
                    {!loading && queryString && (
                        <div className="d-flex align-items-center flex-wrap gap-2">
                            ...
                            {maxResults > 0 && (
                                <span className={classes.resultsTitle}>of</span>
                            )}
                            <span className={classes.resultsTitleBold}>
                                {maxResults}
                            </span>
                    ...
                    {loading && (
                        <div
                            className="placeholder-glow"
                            style={{
                                height: filter === 'list' ? '1185px' : '1595px',
                            }}
                        >
                            Loading...
                        </div>
