...
function AllView({ queryString, setQuery, setTab, isMobile }: ViewProps) {
    ...
    const groupFq: MappingType = {
        ...
        Datasets:
            'idxtype:DATARESOURCE OR idxtype:DATAPROVIDER OR idxtype:INSTITUTION OR idxtype:COLLECTION',
                ...
                if (
                    ...
                    data.facetResults[0].fieldResult.forEach((facet: any) => {
                        var group: string | undefined =
                            groupMapping[facet.label];
                        if (group) {
                            searchGroups[group].count =
                                searchGroups[group].count + facet.count;
                        }
                    });
                }
                    ...
                    if (group.count > 0) {
                        fetch(
                            import.meta.env.VITE_APP_BIE_URL +
                                ...
                                encodeURIComponent(queryString as string) +
                                '&pageSize=4&fq=' +
                                encodeURIComponent(groupFq[group.label] || '')
                        )
                            .then((response) => response.json())
                            .then((data) => {
