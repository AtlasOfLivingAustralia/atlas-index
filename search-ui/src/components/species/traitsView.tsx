...
function TraitsView({ result }: MapViewProps) {
        ...
        fetch(summaryUrl, {
            ...
            .then((data) => {
                ...
                if (data?.categorical_traits) {
                    data.categorical_traits.forEach(
            ...
            .catch((error) => {
                setErrorMessageSummary(
                    'Traits summary - ' + error + ' - ' + summaryUrl
                );
            })
            .finally(() => {
    ...
    function getAusTraitsParam() {
        if (result?.name) {
            return (
                ...
                (result.guid.includes('apni')
                    ? '&APNI_ID=' +
                      encodeURIComponent(
                          result.guid.split('/')[
                              result.guid.split('/').length - 1
                          ]
                      )
                    : '')
            );
    ...
    return (
        <>
            <div
                className={`${classes.traitsSectionText} ${classes.layoutGrid} row`}
