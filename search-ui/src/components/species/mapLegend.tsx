...
function Legend({fillOpacity, hexBinValues}: { fillOpacity: number; hexBinValues: [string, number | null][]; }) {
    ...
    const extractValues = useCallback(
        (hexBinValues: [string, number | null][]) => {
            ...
            const maxCount =
                Math.max(...hexBinValues.map(([, count]) => count || 0)) * 10;
    ...
    const legendEntries: LegendProps[] = useMemo(() => {
        return Object.keys(colours)
            ...
            .map((count, index) => {
                const lower = (
                    Number(Object.keys(colours)[index - 1] || 0) + 1
                ).toLocaleString();
    ...
    return <div style={{borderRadius: '8px', backdropFilter: 'blur(5px)', marginBottom: '5px', marginLeft: '30px'}}>
        ...
        <div className="row" style={{width: `${gridWidth * maxGridCount}px`}}>
            {legendEntries.map((entry, idx) => (
                <div key={idx} className="col map-legend-label text-center"
                     style={{
