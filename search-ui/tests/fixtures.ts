    ...
    page: async ({ page }, use, testInfo) => {
        ...
        page.on('console', msg => {
            if (msg.type() === 'error' || msg.type() === 'warning') {
                ...
                // metadata console.log is expected — suppress it
                if (text.startsWith('metadata')) return;
                // Expected: tests that simulate a network failure via
                // route.abort() cause the browser to report the aborted
                // cross-origin request as a CORS error (this is how
                // Firefox/Chromium describe any failed cross-origin fetch,
                // not an indication of a missing/incorrect mock — a truly
                // unmocked URL is instead caught by logMissingMocks, which
                // throws and fails the test directly).
