...
const testWithCoverage = baseTest.extend<object, { workerCoverageIndex: number }>({
    ...
    page: async ({ page }, use, testInfo) => {
        ...
        // Forward browser console errors and warnings to the Node.js output.
        // In live mode this makes WAF blocks (403s, redirects) and JS errors
        // visible in the test report without needing to open a trace file.
        // Only surfaces errors originating from the app URL — external pages
        // navigated to by click-tests generate their own unrelated noise.
        const appOrigin = new URL(getBaseUrl()).origin;
        page.on('console', msg => {
            if (msg.type() === 'error' || msg.type() === 'warning') {
                const text = msg.text();
                // Suppress GPU/WebGL performance warnings — Mac-specific rendering noise.
                if (text.includes('GPU stall') || text.includes('WebGL')) return;
                // Suppress font rendering warnings — these come from external pages.
                if (text.includes('downloadable font') || text.includes('glyph')) return;
                // Only log messages from the app's own origin.
        ...
        // Log failed network requests originating from the app (not external destinations).
        // NS_BINDING_ABORTED, ERR_ABORTED, and "cancelled" are browser-initiated
        // cancellations when navigating away — not real failures.
        ...
        // Log non-ok HTTP responses from the app that may indicate WAF blocks or server errors.
...
//   expect(count, 'regions.occurrenceCount').toBeGreaterThan(1);
//
