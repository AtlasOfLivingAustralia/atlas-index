...
function buildRegionsList() {
    ...
    const hash = createHash('sha256')
        .update(regionsListContent)
        .digest('hex')
        .slice(0, 8);
    const file = `regionsList-${hash}.json`;
    writeFileSync('./' + file, regionsListContent);
    writeFileSync(
        './regionsList.meta.json',
        JSON.stringify({
            modified: new Date(),
            file: file,
            baseSpatialUrl: baseSpatialUrl,
        })
    );
    console.log(
        `${file} built successfully. It will be used by the next 'yarn run build'.`
    );
}
