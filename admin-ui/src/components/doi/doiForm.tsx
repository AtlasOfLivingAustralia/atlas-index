...
import {UserInfo} from "@ala/common-ui";
...
function DoiForm({onSubmit}: DoiFormProps) {
    ...
    const [userInfo, _] = useState<UserInfo | null>(null);
    ...
    return (
        <Form onSubmit={handleSubmit}>
            ...
            <Form.Group className="mb-3">
                <Form.Label>Provider Metadata (JSON) <span
                    className="text-danger">* {!providerMetadataValid && "invalid"}</span></Form.Label>
                <Form.Control as="textarea" rows={12} value={providerMetadata}
            ...
            <Form.Group className="mb-3">
                <Form.Label>Application URL <span
                    className="text-danger">* {!applicationUrlValid && "invalid"}</span></Form.Label>
                <Form.Control type="text" value={applicationUrl} onChange={e => setApplicationUrl(e.target.value)}
                              required/>
            </Form.Group>
            <Form.Group className="mb-3">
                <Form.Label>Custom Landing Page URL</Form.Label>
                <Form.Control type="text" value={customLandingPageUrl}
                              onChange={e => setCustomLandingPageUrl(e.target.value)}/>
            </Form.Group>
            <Form.Group className="mb-3">
                <Form.Label>Application Metadata {!applicationMetadataValid &&
                    <span className="text-danger">invalid</span>}</Form.Label>
                <Form.Control as="textarea" rows={2} value={applicationMetadata}
                              onChange={e => setApplicationMetadata(e.target.value)}/>
            </Form.Group>
            ...
            <Form.Group className="mb-3">
                ...
                <Form.Control type="text" value={fileUrl} onChange={e => setFileUrl(e.target.value)}
