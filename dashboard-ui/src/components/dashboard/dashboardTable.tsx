...
const DashboardTable = ({
            ...
            <thead>
                <tr>
                    ...
                    <th className="text-end">
                        {header[1] && <FormattedMessage id={header[1]} />}
                    </th>
                    {header[2] && (
                        <th className="text-end ps-1">
                            <FormattedMessage id={header[2]} />
                        </th>
                    )}
                    {header[3] && (
                        <th className="text-end ps-1">
                            <FormattedMessage id={header[3]} />
                        </th>
                    )}
                    {header[4] && (
                        <th className="text-end ps-1">
                            <FormattedMessage id={header[4]} />
                        </th>
                    )}
                </tr>
            </thead>
    ...
    return (
        <table className="dashboardTable" style={{ width: '100%' }}>
            {headerElement}
            <tbody>
                {rows.map((row: TableRow, index: number) => {
                    ...
                    return (
                        <tr
                            className={rowClass}
                            key={index}
                            onClick={() => row.url && clickRow(row.url)}
                        >
                            <td>
                                {!italisizeName ? (
                                    row.name && (
