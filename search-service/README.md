...
| hidden-images-lists.json:.lists[0].uid       | deprecated                                  |                                                                          |                                                                                                                                                          |
| hidden-images-lists.json:.lists[0].imageId   | deprecated                                  |                                                                          | Prepare before importing all data, or manually execute the database sync after transfering the old list data into postgres. Now managed in the Admin UI. |
...
| images-lists.json:.lists[*].uid              | lists.images.preferred.id comma separated   |                                                                          | conflicts with other image methods, best not to use or import into postgres                                                                              |
| images-lists.json:.lists[*].imageId          | lists.images.preferred.field                | imageId (unchanged)                                                      | should be "imageId" but default is "image Id"                                                                                                            |
...
| wiki-lists.json:.lists[*].uid                | deprecated                                  |                                                                          | Prepare before importing all data, or manually execute the database sync after transfering the old list data into postgres. Now managed in the Admin UI. |
| wiki-lists.json:.lists[*].wikiUrl            | deprecated                                  |                                                                          |                                                                                                                                                          |
...
- biocache; occurrence count, occurrence image + lists preferred image
    ...
    ```
- lists; iconic species, conservation status (COMMON), species attributes
    ```properties
    lists.search.max=1000
    lists.favourite.config=dr781,iconic;dr4778,interest
    lists.conservation.statusField=status
    ```
- lists data to migrate to postgres from species list application; preferred images, hidden images, wiki URLs.
    ```
    - Download the CSV from the lists application
