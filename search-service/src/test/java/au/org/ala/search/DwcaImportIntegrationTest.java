...
public class DwcaImportIntegrationTest extends AbstractIntegrationTestContainers {
    ...
    void searchFreeText_kangaroo_returnsResults() {
        EntityExchangeResult<Map<String, Object>> resp = search(b -> b.queryParam("q", "kangaroo"));
        assertThat(resp.getStatus()).isEqualTo(HttpStatus.OK);
        long total = totalRecords(resp.getResponseBody());
    ...
    void pagination_page0_and_page1_returnDifferentRecords() {
        EntityExchangeResult<Map<String, Object>> page0 = search(b -> b
                .queryParam("q", "kangaroo")
                .queryParam("fq", "idxtype:TAXON")
                .queryParam("pageSize", "2")
                .queryParam("page", "0"));
        EntityExchangeResult<Map<String, Object>> page1 = search(b -> b
                .queryParam("q", "kangaroo")
                .queryParam("fq", "idxtype:TAXON")
                .queryParam("pageSize", "2")
                .queryParam("page", "1"));
        assertThat(page0.getStatus()).isEqualTo(HttpStatus.OK);
        assertThat(page1.getStatus()).isEqualTo(HttpStatus.OK);

        List<Map<String, Object>> results0 = searchResults(page0.getResponseBody());
        List<Map<String, Object>> results1 = searchResults(page1.getResponseBody());
    ...
    void sort_byScientificName_desc_firstGreaterThanAscFirst() {
        EntityExchangeResult<Map<String, Object>> asc = search(b -> b
                .queryParam("q", "kangaroo")
                .queryParam("fq", "idxtype:TAXON")
                .queryParam("pageSize", "20")
                .queryParam("sort", "scientificName")
                .queryParam("dir", "asc"));
        EntityExchangeResult<Map<String, Object>> desc = search(b -> b
                .queryParam("q", "kangaroo")
                .queryParam("fq", "idxtype:TAXON")
                .queryParam("pageSize", "20")
                .queryParam("sort", "scientificName")
                .queryParam("dir", "desc"));
        List<String> namesAsc = searchResults(asc.getResponseBody()).stream()
                .map(r -> (String) r.get("scientificName")).filter(n -> n != null).toList();
        List<String> namesDesc = searchResults(desc.getResponseBody()).stream()
                .map(r -> (String) r.get("scientificName")).filter(n -> n != null).toList();
    ...
    void totalRecords_allIdxtypes_greaterThanTaxonAlone() {
        long taxonTotal = totalRecords(search(b -> b
                .queryParam("q", "kangaroo")
                .queryParam("fq", "idxtype:TAXON")
                .queryParam("pageSize", "0")).getResponseBody());
    ...
    void invalidQuery_returnsErrorResponse() {
        ...
        // V2Controller catches it and returns 400. RestTestClient returns the raw response.
        ...
        EntityExchangeResult<String> resp = restTestClient.get()
                .uri(URI.create("/v2/search?q=field%3A%5BINVALID"))
                .exchange()
                .expectBody(String.class)
                .returnResult();
        assertThat(resp.getStatus().is4xxClientError()).isTrue();
    }
